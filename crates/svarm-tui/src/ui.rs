...
pub fn usage_button_area(area: Rect, sidebar_visible: bool) -> Option<Rect> {
    let new_agent = new_agent_button_area(area, sidebar_visible)?;
...
pub(crate) fn click_action(app: &App, area: Rect, column: u16, row: u16) -> Option<ClickAction> {
    ...
    if matches!(
        ...
        if app.mode() != Mode::Menu
            && usage_button_area(area, app.sidebar_visible())
...
fn render_sidebar(
    ...
    if let Some(usage_button) = usage_button_area(area, true) {
...
mod tests {
    ...
    fn menu_hit_areas_stay_at_the_bottom_of_the_sidebar() {
        ...
        assert_eq!(usage_button_area(area, true), Some(Rect::new(0, 37, 27, 1)));
        assert_eq!(
        ...
        assert_eq!(usage_button_area(area, false), None);
    ...
    fn sidebar_renders_new_agent_button_above_menu() {
        ...
        let usage_button = row(usage_button_area(area, true).unwrap());
    ...
    fn usage_hints_and_the_sidebar_button_are_clickable() {
        ...
        let button = usage_button_area(area, true).unwrap();
        assert_eq!(
    ...
    fn multiline_agent_list_scrolls_to_keep_the_selected_card_visible_at_80x24() {
        ...
        app.scroll_sidebar(-8, agent_list_page_size(&app, area));
        let rendered = render_app_text(&app);
